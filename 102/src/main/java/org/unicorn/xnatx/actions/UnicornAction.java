package org.unicorn.xnatx.actions;
...
@Slf4j
@Service
public class UnicornAction extends SingleActionProvider {

    private String displayName = "Unicorn Logging Action";
    private String description = "Unicorn Simple action for EventService Event that logs event detection.";
    ...
    public UnicornAction() {
        log.error("UnicornAction constructor");
    }
    ...
    public void processEvent(EventServiceEvent event, Subscription subscription, UserI user, final Long deliveryId) {
        log.info("UnicornAction called for Subscription {}", subscription.name());
        log.error("UnicornAction called for Subscription {}", subscription.name());
        try {
            ...
            if(serializableObject != null){
                ...
                log.error("Subscription: {}", mapper.writeValueAsString(subscription));
                log.error("Event: {}", event.toString());
                log.error("Event Payload: {}", mapper.writeValueAsString(serializableObject));
            }
